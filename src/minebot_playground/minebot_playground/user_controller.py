...
#SUGESTÃO: ENVIAR OS COMANDOS VIA SERAL UART
class CommandsListenerNode(Node):
    def __init__(self):
        super().__init__('listener_commands')
        # --- setup linear and angular value
        self.__linear_value = 0
        self.__angular_value = 0
        # --- setup subcription
        self.subscriber = self.create_subscription(Twist, 'robot_commands', self.listener_callback, 10)
