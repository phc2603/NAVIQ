...
class CameraNode(Node):
    def __init__(self, cam):
        ...
        self.cap = cv2.VideoCapture(0)  # Open the specified webcam
        ...
        self.cap.set(cv2.CAP_PROP_FPS, 30)  # Set the frames per second to 60
    ...
    def timer_callback(self):
        ...
        if ret:  # Check if the frame was captured successfully
            ...
            self.get_logger().info("Cam capture!!")
            if cv2.waitKey(1) & 0xFF == ord('q'):  # Check if the 'q' key is pressed
                rclpy.shutdown()  # Shut down the ROS 2 node
        else:
            self.get_logger().error('Failed to capture image- no image found')  # Log an error message if the frame was not captured
